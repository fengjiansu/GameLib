...
static int _gamelib_load_bmp_memory_into_sprite(GameLib *game, const void *data, int size)
{
    if (!game || !data || size < 54) return -1;

    const unsigned char *bytes = (const unsigned char*)data;
    if (bytes[0] != 'B' || bytes[1] != 'M') return -1;

    int dataOffset; memcpy(&dataOffset, &bytes[10], 4);
    int dibSize;    memcpy(&dibSize,    &bytes[14], 4);
    int width;      memcpy(&width,      &bytes[18], 4);
    int height;     memcpy(&height,     &bytes[22], 4);
    short bppShort; memcpy(&bppShort,   &bytes[28], 2);
    int bpp = bppShort;
    int colorsUsed; memcpy(&colorsUsed, &bytes[46], 4);
    ...
    if (height < 0) height = -height;
    if (width <= 0 || height <= 0 || width > 16384 || height > 16384) return -1;
    if (dataOffset < 0 || dataOffset > size) return -1;
    ...
    if (bpp == 8) {
        ...
        int palBytes = paletteCount * 4;
        if (paletteOffset < 0 || paletteOffset > size || palBytes > size - paletteOffset) return -1;
        const unsigned char *palData = bytes + paletteOffset;
        for (int i = 0; i < paletteCount; i++) {
            unsigned char b = palData[i * 4 + 0];
            unsigned char g = palData[i * 4 + 1];
            unsigned char r = palData[i * 4 + 2];
    ...
    if (dataOffset > size || rowSize > (size - dataOffset) / height) return -1;

    int id = game->CreateSprite(width, height);
    if (id < 0) return -1;

    for (int y = 0; y < height; y++) {
        const unsigned char *rowData = bytes + dataOffset + y * rowSize;
        int destY = bottomUp ? (height - 1 - y) : y;
        for (int x = 0; x < width; x++) {
            uint32_t color;
            if (bpp == 8) {
                color = palette[rowData[x]];
            } else {
                unsigned char b = rowData[x * bytesPerPixel + 0];
                unsigned char g = rowData[x * bytesPerPixel + 1];
                unsigned char r = rowData[x * bytesPerPixel + 2];
            ...
            game->SetSpritePixel(id, x, destY, color);
        }
    }
...
int GameLib::LoadSpriteMemory(const void *data, int size)
{
    ...
    int imgW = 0, imgH = 0;
    uint32_t *pixels = _gamelib_gdiplus_load(data, size, &imgW, &imgH);

    if (!pixels || imgW <= 0 || imgH <= 0) {
        if (pixels) free(pixels);
        const unsigned char *bytes = (const unsigned char*)data;
        bool isBMP = (size >= 2 && bytes[0] == 'B' && bytes[1] == 'M');
        return isBMP ? _gamelib_load_bmp_memory_into_sprite(this, data, size) : -1;
    }
...
int GameLib::LoadSprite(const char *filename)
{
    ...
    if (fileSize <= 0 || fileSize > 0x7fffffffL) { fclose(fp); return -1; }
    ...
    int id = LoadSpriteMemory(fileData, (int)fileSize);
    free(fileData);
    return id;
}
...
static uint16_t _gamelib_read_u16le(const unsigned char *p)
{
    return (uint16_t)((uint16_t)p[0] | ((uint16_t)p[1] << 8));
}

static uint32_t _gamelib_read_u32le(const unsigned char *p)
{
...
GameLib::_WavData *GameLib::_LoadWAVFromMemory(const void *data, int size)
{
    ...
    while (offset + 8 <= size) {
        ...
        uint32_t chunk_size = _gamelib_read_u32le(chunk + 4);
        ...
        if (chunk[0] == 'f' && chunk[1] == 'm' &&
            chunk[2] == 't' && chunk[3] == ' ') {
            if (chunk_size >= 16) {
                audio_format = _gamelib_read_u16le(bytes + data_offset + 0);
                channels = _gamelib_read_u16le(bytes + data_offset + 2);
                sample_rate = _gamelib_read_u32le(bytes + data_offset + 4);
                bits_per_sample = _gamelib_read_u16le(bytes + data_offset + 14);
            }
